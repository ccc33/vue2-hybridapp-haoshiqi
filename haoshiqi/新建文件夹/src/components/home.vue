	...
	<script>
		...
		export default {
			created(){
				...
				{
					//Toast(`开始定位`);
					navigator.geolocation.getCurrentPosition(onSuccess, onError,{ maximumAge: 3000, timeout: 8000, enableHighAccuracy: true });
				}
				else{
					Toast(`您的设备不支持该定位功能喔`);
				}
			...
			beforeRouteLeave(to,from,next){
				this.loading = true; //禁用无限加载， fixed bug by cxl
				next();
			},
